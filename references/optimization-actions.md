...
| `QUANTIFY` | Add or strengthen sourced metrics | The original resume or user supplied a measurable outcome | "Improved performance" → "Improved API response time by 65% (800ms → 280ms)" when those values are sourced |
...
- `QUANTIFY` must use real numbers from the original resume or user. Never infer or invent metrics.
